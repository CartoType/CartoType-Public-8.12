...
BOOL CCartoTypeDemoDoc::OnOpenDocument(LPCTSTR lpszPathName)
    ...
    // Uncomment the next two lines, and supply a valid Mapbox key, to load Mapbox data.
    //else if (filename.First(6) == "https:")
    //    m_map_data_set = CartoType::FrameworkMapDataSet::New(error,app->Engine(),filename,"<mapboxkey>");
    else
